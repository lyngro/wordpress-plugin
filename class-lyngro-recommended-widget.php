 ...
 *
 * This provides a widget to put on a page, will have lyngro recommended articles
 *
 * @category   Components
 * @package    WordPress
 * @subpackage LynGro
 ...
 *
 * @category   Class
 * @package    Lyngro_Recommended_Widget
 */
class Lyngro_Recommended_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		$widget_options = array(
			'classname'   => 'Lyngro_Recommended_Widget',
			'description' => 'Lyngro recommendation widget',
		);
		parent::__construct( 'Lyngro_Recommended_Widget', 'Lyngro Recommended Widget', $widget_options );
	}
	...
	public function widget( $args, $instance ) {
		...
		$options = get_option( 'lyngro' );
		if ( array_key_exists( 'apikey', $options ) && array_key_exists( 'api_secret', $options ) && ! empty( $options['api_secret'] ) ) {
			$root_url       = 'https://widgets.lyngro.com/widget?v=1&widgetid=' .  $instance['widgetid'] .'&appdomain=' . $options['apikey'] .'&appkey=' . $options['api_secret']  ;
			...
			<script data-cfasync="false">
				...
				function widgetLoad() {
					...
					var cookieVal = document.cookie.replace(/(?:(?:^|.*;\s*)kryptonzpixel2\s*\=\s*([^;]*).*$)|^.*$/, "$1");
					if ( cookieVal ) {
						var uuid =cookieVal;// JSON.parse(unescape(cookieVal));//['id'];
					}

					var full_url = '<?php echo $full_url; ?>';//esc_js

					var personalized = '<?php echo esc_js( boolval( $instance['personalize_results'] ) ); ?>';
					if ( personalized && uuid ) {
						full_url += '&userid=';
						full_url += uuid;
						full_url += '&currentposturl=';
					...
					if (parentDiv.length === 0) {
						parentDiv = jQuery.find('.Lyngro_Recommended_Widget');
					}
					var outerDiv = jQuery('<div>').addClass('lyngro-recommendation-widget').appendTo(parentDiv);
					<?php
					...
					?>
					var outerList = jQuery('<ul>').addClass('lyngro-recommended-widget').appendTo(outerDiv);
					jQuery.getJSON( full_url, function (data) {
						jQuery.each(data.data, function(key, value) {
							var widgetEntry = jQuery('<li>')
								.addClass('lyngro-recommended-widget-entry')
								.attr('id', 'lyngro-recommended-widget-item' + key);
							<?php
							if ( in_array( 'display_thumbnail', $instance['display_options'], true ) ) {
								?>
							var thumbnailImage = jQuery('<img>').attr('src', value['kryptonz_thumb']).appendTo(widgetEntry);
								<?php
							}
							?>
							...
							if ( in_array( 'display_author', $instance['display_options'], true ) ) {
								?>
							var authorDiv = jQuery('<div>').addClass('lyngro-title-author-wrapper');
							var authorLink = jQuery('<a>').attr('href', value['url']).text(value['type']);
			...
			?>
			<p>
			you must set the Lyngro API Secret for this widget to work!
			</p>
			<?php
		}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'widgetid' ) ); ?>">Widget Id:</label>
			<br>
			<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'widgetid' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'widgetid' ) ); ?>" value="<?php echo esc_attr( $widgetid ); ?>" />
		</p>
		<p>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['widgetid']               = trim( $new_instance['widgetid']);
 ...
 * @subpackage LynGro
 */
function lyngro_recommended_widget_register() {
	register_widget( 'Lyngro_Recommended_Widget' );
}

add_action( 'widgets_init', 'lyngro_recommended_widget_register' );
